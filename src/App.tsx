	...
	players: [
		{
			...
			snake: [
				defaultPlayer1Location,
				{ ...defaultPlayer1Location, x: defaultPlayer1Location.x - 1 },
				{ ...defaultPlayer1Location, x: defaultPlayer1Location.x - 2 },
			],
		},
	],
};
...
function App() {
	...
	const handleOnKeyDown = (e: KeyboardEvent) => {
		if (e.key === "w") {
			...
			let newPlayer = gameState.players[0];
			newPlayer.direction = Direction.UP_LEFT;
			setGameState({ ...gameState, players: [newPlayer] });
		} else if (e.key === "e") {
			let newPlayer = gameState.players[0];
			newPlayer.direction = Direction.UP_RIGHT;
			setGameState({ ...gameState, players: [newPlayer] });
		} else if (e.key === "z") {
			let newPlayer = gameState.players[0];
			newPlayer.direction = Direction.DOWN_LEFT;
			setGameState({ ...gameState, players: [newPlayer] });
		} else if (e.key === "c") {
			let newPlayer = gameState.players[0];
			newPlayer.direction = Direction.DOWN_RIGHT;
			setGameState({ ...gameState, players: [newPlayer] });
		}
	};
	useEffect(() => {
	...
	const updateGameState = () => {
		...
		for (let i = 0; i < newGameState.players!.length; i++) {
			...
			if (
				newLocation.x < 0 ||
				newLocation.x >= newGameState.totalGrid! ||
				newLocation.y < 0 ||
				newLocation.y >= newGameState.totalGrid!
			) {
				alert(`Player ${i} Lose`);
			...
			if (isEatFood) {
				...
				let newFood = getNewUniqueCoor(
					[...remainFoods, ...playerCoordinates],
					newGameState.totalGrid!,
					newGameState.totalGrid!
				);
	...
	useEffect(() => {
		let interval = setInterval(updateGameState, 500);
	...
	return (
		...
		>
			...
			<button
				onClick={() =>
					setGameState({ ...gameState, gameState: GameState.START })
				}
			>
				START
			</button>
